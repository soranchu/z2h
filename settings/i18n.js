// SAMPLE
this.i18n = {
    "settings": {
        "en": "Settings",
        "ja": "Settings"
    },
    "information": {
        "en": "Information",
        "ja": "Information"
    },
    "details": {
        "en": "Details",
        "ja": "さよなら全角英数の設定"
    },
	"about_text": {
        "en": "This extension replaces multibyte alphabets, numerals and symbols with single-byte ones.",
        "ja": "この拡張機能は、ページ内のすべての全角英数文字を半角に置換します。あの電話会社のプレスリリースも、あの新聞社のニュースサイトも、キレることなく快適に閲覧できるでしょう。"
    },
	"updates_text": {
        "ja": "2012/07/04 さよなら全角英数の設定を複数の Chrome で同期するようになりました。<br>" +
        	  "2012/06/29 https サイトと Ajax で動的に更新されるページでの変換に対応しました。"
    },
	"replace": {
        "ja": "変換の設定"
    },
	"replace_about_text": {
        "ja": "設定の変更は新しく表示するページから有効になります。"
    },
	"replace_alpha": {
        "ja": "全角アルファベット ( Ａ～Ｚ、ａ～ｚ ) を半角に変換する"
    },
	"replace_num": {
        "ja": "全角数字 ( ０～９ ) を半角に変換する"
    },
	"replace_kana": {
        "ja": "半角カナを全角に変換する "
    },
	"replace_sym": {
        "ja": "以下の全角記号を半角に変換する"
    },
	"open_replace_sym_option": {
        "ja": "詳細を開く"
    },
	"close_replace_sym_option": {
        "ja": "詳細を閉じる"
    },
	"replace_tilde": {
        "ja": "全角チルダ(～)を半角に変換する"
    },
	"replace_space": {
        "ja": "全角スペースを半角に変換する"
    },
	"keep_heading_space": {
        "ja": "行頭の全角スペースは変換しない "
    },
	"support_ajax": {
        "ja": "Ajax による動的な書き換えも対象にする "
    },
	"support_https": {
        "ja": "https のサイトも対象にする "
    },
	"ignore_settings": {
        "ja": "変換を行わないURL"
    },
	"ignore_desc_text": {
        "ja": "変換を行わないページ、ドメインを管理できます。ひとつのURLにつき1行で入力してください。"
    },
	"ignore_pages": {
        "ja": "ページ指定"
    },
	"ignore_domains": {
        "ja": "ドメイン指定"
    },
    "apply":{
    	"ja" : "適用"
    }

};
